...
contract ReceiptVerification is Ownable, ReentrancyGuard {
    ...
    function approveReceipt(
        ...
        onlyVendorBeforeDeadline(_receiptId)
        nonReentrant
    {
    ...
    /**
     * @dev Cancel a receipt request (by vendor)
        ...
        nonReentrant
    {
        ...
        // Only if the receipt was approved AND the canceller is the vendor, decrement totalReceipts
        if (
            receipt.status == ReceiptStatus.Approved &&
            msg.sender == receipt.issuer
        ) {
    ...
    function getPendingRequests(
        ...
        uint256 len = allReceipts.length;
